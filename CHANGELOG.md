...
### Added
- **Scan presets.** Named bundles of scan settings (detection method, mode, thorough decode duration / timeout, hardware acceleration) that you can apply to a scan path with one click. Ships with five built-in presets covering the common cases: **Zero-byte only** (instant first-pass on archived storage), **Quick** (the existing factory default), **Fast triage** (decode first 60s with hwaccel - the "just check the start of every file" preset), **Deep scan** (full decode with a 30-min timeout), and **Paranoid** (HandBrake's stricter decoder, software-only). You can also create your own custom presets under Advanced → Scan presets. Applying a preset writes its values into the scan path form; you can still tweak any field afterwards. Built-in presets are read-only (delete/edit return 403 server-side and are disabled in the UI) so the dropdown can't be accidentally emptied. Phase 3 of the /config redesign; the redesign is now complete.
