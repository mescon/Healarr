...
### Added
- **AV1, VP9, and VP8 thorough scans now actually engage the GPU on NVIDIA, Intel QSV, and VAAPI hosts (Intel/AMD).** Previously, even with a working hwaccel context, ffmpeg picked its software default decoder (`libdav1d` for AV1, `libvpx-vp9` for VP9, `libvpx` for VP8) and the GPU context was set up but never consumed. Healarr now probes the input codec via a cheap ffprobe and adds a vendor-appropriate `-c:v` override that engages the actual hardware decoder. Vendor coverage: NVIDIA `*_cuvid`, Intel QSV `*_qsv`, and VAAPI (which uses ffmpeg's bare internal decoder names + `-hwaccel vaapi` hooks). When `HEALARR_HEALTH_CHECK_HWACCEL=auto`, Healarr detects the vendor from `/dev/nvidiactl` (NVIDIA) or `/dev/dri/renderD*` (VAAPI fallback). H.264 / HEVC / MPEG2 / VC-1 don't need explicit overrides because their default decoders already have internal hwaccel hooks. The retry-without-hwaccel safety net (added in #278) catches any decoder failure on any vendor and falls back to software, so an unexpected hardware-decode crash never routes a file to remediation. Closes [#276](https://github.com/mescon/Healarr/issues/276).

### Changed
- **Docker base image switched from Alpine + custom-compiled ffmpeg to Debian-slim + jellyfin-ffmpeg.** On Alpine the NVIDIA Container Toolkit was failing to inject `libcuda.so` / `libnvcuvid.so` into the container despite `runtime: nvidia` being set — every cuvid decoder SIGSEGV'd in 1-2 seconds, leaving AV1 / VP9 stuck on software decode forever. The same toolkit works perfectly for jellyfin-ffmpeg on Debian (the build Tdarr uses), so we adopted that. ffmpeg version bumps from 7.1.1 to 7.1.4-Jellyfin; hwaccel list grows from `cuda/vaapi/drm` to `cuda/vaapi/qsv/drm/opencl/vulkan`; AV1 NVDEC is now actually functional. Image size grows ~371 MB → ~908 MB, the cost of a working out-of-the-box NVIDIA path. Closes the main symptom of [#276](https://github.com/mescon/Healarr/issues/276).

### Fixed
- **Thorough decode failures that look like a broken GPU runtime (SIGSEGV, libcuda missing, "Failed to setup hwaccel", "Device creation failed", etc.) now retry the same file with hardware acceleration disabled instead of letting the failure flow into the corruption classifier.** Without this guard, a misconfigured driver / missing userspace driver lib / decoder crash would route every affected file straight to the remediation pipeline (delete + re-grab from the *arr) — the silent mass-data-loss failure mode [#276](https://github.com/mescon/Healarr/issues/276) documents. The retry is one extra ffmpeg invocation in the worst case, and only triggers on failure-message patterns that strongly suggest the decoder rather than the file. Healarr now treats GPU-runtime breakage as a transient infrastructure issue, not as evidence of file corruption.
